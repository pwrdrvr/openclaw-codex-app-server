...
class StdioJsonRpcHarness {
  ...
  async initialize() {
    const result = await this.request("initialize", {
      ...
      clientInfo: {
        name: "openclaw-codex-app-server-smoke",
        version: "0.0.0",
      },
      capabilities: {
  ...
  async handleNotification(method, params) {
    ...
    this.turnId ||= ids.runId;
    if (methodLower === "turn/completed") {
      this.turnCompleted = true;
      this.log(`turn completed thread=${this.threadId || "<none>"} turn=${this.turnId || "<none>"}`);
      return;
    }
    if (methodLower === "turn/failed" || methodLower === "turn/cancelled") {
      ...
      this.log(`turn terminal ${method}`);
      return;
    }
    const assistant = extractAssistantNotificationText(method, params);
    if (assistant.mode === "delta" && assistant.text) {
      this.assistantText =
        this.assistantText && assistant.text.startsWith(this.assistantText)
          ? assistant.text
          : `${this.assistantText}${assistant.text}`;
      return;
    }
    if (assistant.mode === "snapshot" && assistant.text.trim()) {
      this.assistantText = assistant.text.trim();
      this.log(`assistant snapshot ${summarizeTextForLog(this.assistantText)}`);
    }
  }
...
async function runScenario({ label, args, cwd, prompt, timeoutMs }) {
  ...
  try {
    ...
    const threadResult = await harness.request(
      "thread/start",
      { cwd },
      15_000,
    );
    harness.threadId ||= extractIds(threadResult).threadId;
    if (!harness.threadId) {
      throw new Error("thread/start did not return a thread id");
    }
    harness.log(`thread started thread=${harness.threadId}`);
    ...
    harness.log(`turn started prompt=${summarizeTextForLog(prompt)}`);

    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      if (harness.approvalRequest) {
        return {
          kind: "approval-requested",
          approvalMethod: harness.approvalRequest.method,
          assistantText: harness.assistantText.trim() || undefined,
          threadId: harness.threadId,
          turnId: harness.turnId,
          logs: harness.logs,
        };
      }
      if (harness.turnCompleted) {
        return {
          kind: "turn-completed",
          assistantText: harness.assistantText.trim() || undefined,
          threadId: harness.threadId,
      ...
      if (harness.turnFailed) {
        return {
          kind: "turn-failed",
          failureMethod: harness.turnFailed.method,
          assistantText: harness.assistantText.trim() || undefined,
          threadId: harness.threadId,
          turnId: harness.turnId,
          logs: harness.logs,
...
function formatOutcome(label, outcome) {
  const lines = [
    `${label}: ${outcome.kind}`,
    `  threadId: ${outcome.threadId || "<none>"}`,
    `  turnId: ${outcome.turnId || "<none>"}`,
  ];
  if (outcome.approvalMethod) {
    lines.push(`  approvalMethod: ${outcome.approvalMethod}`);
  }
  if (outcome.failureMethod) {
    lines.push(`  failureMethod: ${outcome.failureMethod}`);
  }
  if (outcome.assistantText) {
    lines.push(`  assistantText: ${summarizeTextForLog(outcome.assistantText, 200)}`);
  }
  return lines.join("\n");
}
...
async function main() {
  ...
  for (const scenario of scenarios) {
    ...
    outcomes.push({ label: scenario.label, outcome });
    console.log(formatOutcome(scenario.label, outcome));
  }

  console.log("\nSummary:");
  for (const { label, outcome } of outcomes) {
    console.log(formatOutcome(label, outcome));
  }
}
